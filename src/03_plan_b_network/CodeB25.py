...
print("RUNNING CODEB25: AF-aware multiplex refinement")
...
class B25Config:
    ...
    # This uses a graded temporal kernel instead of only linking consecutive times.
    ...
    # Bridge score mixture
    # topological bridge = betweenness on supra-graph
    # boundary bridge = phenotype boundary score
    bridge_mix_topology: float = 0.60
    bridge_mix_boundary: float = 0.40
...
def build_weighted_graph_from_adjacency(W: np.ndarray):
    """
    Build a weighted undirected NetworkX graph from adjacency matrix.
    """
...
def top_nodes_by_metric(df: pd.DataFrame, metric_col: str, top_k: int, ascending: bool = False):
    """
    Return top-k nodes by a given metric.
    """
...
def build_temporal_similarity(node_table: pd.DataFrame, sigma_steps: float):
    """
    Build graded temporal similarity from placement time index.
    Observations close in time receive higher similarity.
    """
...
def build_phenotype_similarity(node_table: pd.DataFrame, cols, sigma_f: float):
    """
    Build phenotype similarity using raw Plan A descriptors instead of aggregate scores.
    """
...
def build_refined_layer_adjacency(spatial_W, temporal_W, phenotype_W, config: B25Config):
    """
    Build AF-aware refined layer adjacency by combining:
    - spatial continuity
    - temporal continuity
...
def choose_omega(A_sp: np.ndarray, A_pc: np.ndarray, config: B25Config):
    """
    Choose inter-layer coupling based on mean nonzero intra-layer weight.
    """
...
def weighted_boundary_score(A: np.ndarray, Z: np.ndarray, eps: float = 1e-8):
    """
    Compute a phenotype-boundary score for each node.

    A high boundary score means that a node is connected to neighbors
    with different phenotype vectors, suggesting a transition zone.
    """
    n = A.shape[0]
...
# =========================================================
# 6) Build refined B2.5 table
# =========================================================

def build_b25_role_table(node_table: pd.DataFrame, config: B25Config):
    """
    Main B2.5 pipeline:
    - build AF-aware layer adjacencies
    - compute layer metrics
    - compute topological bridge
    - compute phenotype boundary score
    - combine them into an AF-aware bridge score
    """
    # Raw Plan A phenotype descriptors for each layer
    ...
    # Layer-specific phenotype similarities
    ...
    # Refined layer adjacencies
    A_sp, W_spatial_sp, W_temporal_sp, W_pheno_sp = build_refined_layer_adjacency(
        spatial_W, temporal_W, phenotype_W_sp, config
    )
    A_pc, W_spatial_pc, W_temporal_pc, W_pheno_pc = build_refined_layer_adjacency(
        spatial_W, temporal_W, phenotype_W_pc, config
    )
    ...
    # Topological bridge from supra-graph
    ...
    # Topological bridge normalized
    bet_mean_norm = minmax_array(bet_mean, eps=config.eps)

    # AF-aware bridge score
    af_bridge_score = (
        config.bridge_mix_topology * bet_mean_norm
        + config.bridge_mix_boundary * boundary_mean
    )

    # Keep the previously meaningful core and discordance logic
    ...
    # Optional recency weight for interpretation, not as a hard driver
    ...
    df = pd.DataFrame({
        ...
        "af_bridge_score": af_bridge_score,
    ...
    artifacts = {
        ...
        "W_spatial_sp": W_spatial_sp,
        "W_temporal_sp": W_temporal_sp,
        "W_pheno_sp": W_pheno_sp,
        "W_spatial_pc": W_spatial_pc,
        "W_temporal_pc": W_temporal_pc,
        "W_pheno_pc": W_pheno_pc,
        "omega": omega,
    }
...
def build_b25_summary(df: pd.DataFrame, artifacts: dict, config: B25Config):
    """
    Build a text summary focusing on whether AF-aware refinement improves interpretation.
    """
    top_core = top_nodes_by_metric(df, "core_score", config.top_k_report, ascending=False)
    top_bridge = top_nodes_by_metric(df, "af_bridge_score", config.top_k_report, ascending=False)
    top_boundary = top_nodes_by_metric(df, "boundary_mean_refined", config.top_k_report, ascending=False)

    lines = []
    lines.append("CODEB25 AF-AWARE REFINEMENT SUMMARY")
    lines.append("=" * 64)
    lines.append("")
    ...
    lines.append("Top AF-aware bridge nodes:")
    for _, row in top_bridge.iterrows():
        lines.append(
            f"  placement {int(row['placement_id'])}: af_bridge={row['af_bridge_score']:.4f}, "
            f"bet={row['betweenness_supra_mean_refined']:.4f}, "
            f"boundary={row['boundary_mean_refined']:.4f}"
        )
    lines.append("")

    lines.append("Top phenotype-boundary nodes:")
    for _, row in top_boundary.iterrows():
        lines.append(
            f"  placement {int(row['placement_id'])}: boundary={row['boundary_mean_refined']:.4f}, "
            f"disc={row['signed_discordance']:.4f}"
        )
    lines.append("")
    ...
    lines.append(
        f"  corr(core_score, mean_vuln_field)              = {safe_corr(df['core_score'], df['mean_vuln_field']):.4f}"
    )
    lines.append(
        f"  corr(topological_betweenness, mean_interaction)= {safe_corr(df['betweenness_supra_mean_refined'], df['mean_interaction']):.4f}"
    )
    lines.append(
        f"  corr(boundary_mean, mean_interaction)          = {safe_corr(df['boundary_mean_refined'], df['mean_interaction']):.4f}"
    )
    lines.append(
        f"  corr(af_bridge_score, mean_interaction)        = {safe_corr(df['af_bridge_score'], df['mean_interaction']):.4f}"
    )
    lines.append(
        f"  corr(abs_discordance, mean_vuln_field)         = {safe_corr(df['abs_discordance'], df['mean_vuln_field']):.4f}"
    )
    lines.append(
        f"  corr(abs_discordance, time_index)              = {safe_corr(df['abs_discordance'], df['time_index']):.4f}"
    )
    lines.append(
        f"  corr(af_bridge_score, time_index)              = {safe_corr(df['af_bridge_score'], df['time_index']):.4f}"
    )
    lines.append(
        f"  corr(boundary_mean, time_index)                = {safe_corr(df['boundary_mean_refined'], df['time_index']):.4f}"
    )
    lines.append("")
...
def plot_single_metric_map(df: pd.DataFrame, value_col: str, title: str, out_path: Path,
                           config: B25Config, cmap: str = "viridis",
...
def plot_two_layer_metric_maps(df: pd.DataFrame, col_sp: str, col_pc: str,
                               title_sp: str, title_pc: str, out_path: Path, config: B25Config):
    """
    Plot side-by-side layer maps.
    """
...
def plot_bridge_comparison(df: pd.DataFrame, out_path: Path, config: B25Config):
    """
    Compare topological bridge vs AF-aware bridge in one figure.
    """
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    ...
    ax.set_title("Topological bridge map (refined supra betweenness)")
    ax.set_xlabel("x (mm)")
    ax.set_ylabel("y (mm)")
    fig.colorbar(sc, ax=ax, fraction=0.046, pad=0.04)
    ...
    sc = ax.scatter(
        df["center_x"], df["center_y"],
        c=df["af_bridge_score"],
        s=config.node_size,
        cmap="magma"
    )
    ax.set_title("AF-aware bridge map")
    ax.set_xlabel("x (mm)")
    ax.set_ylabel("y (mm)")
    fig.colorbar(sc, ax=ax, fraction=0.046, pad=0.04)
...
def plot_top_nodes_summary(df: pd.DataFrame, out_path: Path, config: B25Config):
    """
    Create a bar-plot summary of top node roles after AF-aware refinement.
    """
    top_core = top_nodes_by_metric(df, "core_score", config.top_k_report, ascending=False)
    top_bridge = top_nodes_by_metric(df, "af_bridge_score", config.top_k_report, ascending=False)
    top_sp_dom = top_nodes_by_metric(df, "signed_discordance", config.top_k_report, ascending=False)
    top_pc_dom = top_nodes_by_metric(df, "signed_discordance", config.top_k_report, ascending=True)
    ...
    ax = axes[0, 1]
    ax.barh(top_bridge["placement_id"].astype(str), top_bridge["af_bridge_score"])
    ax.set_title("Top AF-aware bridge nodes")
    ax.invert_yaxis()
    ...
    ax = axes[1, 1]
    ax.barh(top_pc_dom["placement_id"].astype(str), top_pc_dom["signed_discordance"])
    ax.set_title("Top paced-dominant nodes")
    ax.invert_yaxis()
...
if __name__ == "__main__":
    ...
    output_dir = script_dir / "codeB25_outputs"
    ...
    # -------------------------
    # Build refined role table
    # -------------------------
    refined_df, artifacts = build_b25_role_table(node_table, config)

    # -------------------------
    # Save refined tables
    # -------------------------
    refined_df.to_csv(output_dir / "b25_node_metrics_refined.csv", index=False)

    node_ids = refined_df["placement_id"].astype(int).tolist()
    pd.DataFrame(
        artifacts["A_sp_refined"], index=node_ids, columns=node_ids
    ).to_csv(output_dir / "b25_spont_adjacency_refined.csv")

    pd.DataFrame(
        artifacts["A_pc_refined"], index=node_ids, columns=node_ids
    ).to_csv(output_dir / "b25_paced_adjacency_refined.csv")
    ...
    ).to_csv(output_dir / "b25_supra_adjacency_refined.csv")

    # -------------------------
    # Build summary text
    # -------------------------
    summary_text = build_b25_summary(refined_df, artifacts, config)
    with open(output_dir / "b25_summary.txt", "w", encoding="utf-8") as f:
    ...
    plot_two_layer_metric_maps(
        df=refined_df,
        col_sp="strength_sp_refined",
        col_pc="strength_pc_refined",
        title_sp="Refined spontaneous node strength",
        title_pc="Refined paced node strength",
        out_path=output_dir / "b25_strength_maps_refined.png",
        config=config
    )

    plot_two_layer_metric_maps(
        df=refined_df,
        col_sp="clustering_sp_refined",
        col_pc="clustering_pc_refined",
        title_sp="Refined spontaneous clustering",
        title_pc="Refined paced clustering",
        out_path=output_dir / "b25_clustering_maps_refined.png",
        config=config
    )
    ...
    plot_single_metric_map(
        ...
        title="Refined core score map",
        out_path=output_dir / "b25_core_map.png",
        config=config,
        cmap="viridis",
        annotate_top=True,
    ...
    plot_single_metric_map(
        df=refined_df,
        value_col="boundary_mean_refined",
        title="Phenotype boundary map",
        out_path=output_dir / "b25_boundary_map.png",
        config=config,
        cmap="cividis",
        annotate_top=True,
        top_metric="boundary_mean_refined",
        ascending=False
    )

    plot_single_metric_map(
        df=refined_df,
        value_col="af_bridge_score",
        title="AF-aware bridge map",
        out_path=output_dir / "b25_af_bridge_map.png",
        config=config,
        cmap="magma",
        annotate_top=True,
        top_metric="af_bridge_score",
        ascending=False
    )

    plot_bridge_comparison(
        df=refined_df,
        out_path=output_dir / "b25_bridge_comparison.png",
        config=config
    )
    ...
    plot_single_metric_map(
        ...
        title="Recency weight map",
        out_path=output_dir / "b25_recency_map.png",
        config=config,
        cmap="plasma",
        annotate_top=False
    )
    ...
    plot_top_nodes_summary(
        df=refined_df,
        out_path=output_dir / "b25_top_nodes_summary.png",
        config=config
    )
    ...
    # -------------------------
    print("\nCodeB25 outputs saved to:")
    print(output_dir)

    print("\nKey files:")
    print(" - b25_node_metrics_refined.csv")
    print(" - b25_spont_adjacency_refined.csv")
    print(" - b25_paced_adjacency_refined.csv")
    print(" - b25_supra_adjacency_refined.csv")
    print(" - b25_summary.txt")
    print(" - b25_strength_maps_refined.png")
    print(" - b25_clustering_maps_refined.png")
    print(" - b25_core_map.png")
    print(" - b25_boundary_map.png")
    print(" - b25_af_bridge_map.png")
    print(" - b25_bridge_comparison.png")
    print(" - b25_recency_map.png")
    print(" - b25_top_nodes_summary.png")
