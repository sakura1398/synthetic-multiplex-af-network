...
# =========================================================
# 11) Full pipeline for one config
# =========================================================
...
def run_pipeline_for_config(config: DatasetConfig, substrate: dict, placement_df: pd.DataFrame):
    ...
    n_masked_valid = int(np.sum(~np.isnan(masked_score)))
    ...
    summary = {
        ...
        "n_masked_valid_pixels": n_masked_valid,
    }

    return patch_summary_df, xs, ys, raw_global, confidence, normalized_score, masked_score, summary
...
def plot_sensitivity_maps(results, output_path: Path):
    n = len(results)
    ...
    for ax, item in zip(axes, results):
        ...
        ax.set_title(
            f"Masked Map | LV thr={config.low_voltage_threshold:.2f}, "
            f"sigma_kernel={config.sigma_kernel:.1f}"
        )
        ax.set_xlabel("x (mm)")
        ax.set_ylabel("y (mm)")
...
if __name__ == "__main__":
    ...
    # Freeze the hidden world and trajectory so comparison is fair
    substrate = generate_hidden_substrate(base_config, rng)
    placement_df = generate_trajectory(base_config, rng)

    # Sensitivity settings
    lv_thresholds = [0.35, 0.50, 0.65]
    sigma_kernels = [4.0, 6.0, 8.0]

    experiment_configs = [
        replace(base_config, low_voltage_threshold=lv, sigma_kernel=sk)
        for lv, sk in zip(lv_thresholds, sigma_kernels)
    ]

    results = []
    summary_rows = []

    output_dir = Path("manual_check_sensitivity")
    output_dir.mkdir(parents=True, exist_ok=True)

    for i, cfg in enumerate(experiment_configs):
        patch_df, xs, ys, raw_global, confidence, normalized_score, masked_score, summary = run_pipeline_for_config(
            config=cfg,
            substrate=substrate,
        ...
        patch_df.to_csv(output_dir / f"patch_summary_run_{i}.csv", index=False)
        pd.DataFrame(masked_score, index=ys, columns=xs).to_csv(output_dir / f"masked_map_run_{i}.csv")

    summary_df = pd.DataFrame(summary_rows)
    summary_df.to_csv(output_dir / "sensitivity_summary.csv", index=False)

    plot_sensitivity_maps(results, output_dir / "sensitivity_masked_maps.png")

    print("Sensitivity-analysis files saved to: manual_check_sensitivity/")
