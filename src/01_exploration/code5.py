...
def compute_global_maps(patch_df: pd.DataFrame, config: DatasetConfig, eps: float = 1e-8):
    ...
    return xs, ys, X, Y, raw_global, confidence, normalized_score
...
def save_global_map_figure(output_dir: str,
                           config: DatasetConfig,
    ...
    fig.tight_layout()
    fig.savefig(out / "global_maps.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
...
def run_basic_checks(electrode_df: pd.DataFrame,
                     patch_df: pd.DataFrame,
                     raw_global: np.ndarray,
                     confidence: np.ndarray,
                     normalized_score: np.ndarray):
    ...
    print("NormalizedScore min/max:", float(normalized_score.min()), "to", float(normalized_score.max()))
...
if __name__ == "__main__":
    ...
    xs, ys, X, Y, raw_global, confidence, normalized_score = compute_global_maps(
        patch_df=patch_df,
        config=config
    )

    output_dir = "manual_check_global_maps"
    ...
    print("Global-map files saved to: manual_check_global_maps/")
    ...
    run_basic_checks(electrode_df, patch_df, raw_global, confidence, normalized_score)